...
export default function Play2({ navigation }) {
    ...
    // Réinitialiser le garde-fou quand une nouvelle partie démarre
    ...
    useEffect(() => {
        ...
        const aliveUsers = (users || []).filter(u => !u.isDead);
        const hackers = aliveUsers.filter(u => u.role === "hacker");
        const others = aliveUsers.filter(u => u.role !== "hacker");

        const endGame = (winnerRole) => {
            hasEndedRef.current = true;
            try { socket?.emit?.("stopGame"); } catch (e) { }
            // replace évite de revenir en arrière vers la partie finie

            navigation.replace("GameOver", {
                result: "victory",
                rolewin: winnerRole,  // "dev" ou "hacker"
                xpEarned: 100,
                role: myRole,
            });
        ...
        // 0 ou 1 survivant => on décide le gagnant selon le dernier vivant
        if (aliveUsers.length <= 1) {
            const last = aliveUsers[0];
            endGame(last?.role === "hacker" ? "hacker" : "dev");
            return;
        }

        // Plus aucun hacker vivant => devs gagnent
        if (hackers.length === 0) {
            endGame("dev");
            return;
        }

        // Parité ou majorité hacker => hackers gagnent
        if (hackers.length >= others.length) {
            endGame("hacker");
            return;
        }
    }, [users, gameStarted, connected, navigation, socket, myRole]);
